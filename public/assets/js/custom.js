...
$(document).ready(function () {
    ...
    if ($menu.length === 0) return; // Exit if menu not found
    ...
    function getMenuItemKey($item) {
        // Try to get text from the div inside the a tag
        var text = $item.find('a div').text().trim();
        if (text) return text;

        // If no text, try to get a unique class (assuming each menu item has a unique class)
        var classes = $item.attr('class').split(/\s+/);
        for (var i = 0; i < classes.length; i++) {
            if (classes[i] !== 'menu-item' && classes[i] !== 'active') {
                return classes[i];
            }
        }

        // If all else fails, use the href
    ...
    function reorderMenu() {
        ...
        $menuItems.detach().sort(function (a, b) {
            ...
            return (usageCounts[keyB] || 0) - (usageCounts[keyA] || 0);
        });
        $menu.append($menuItems).css('visibility', 'visible');
    }
    ...
    $menuItems.find('a').on('click', function (e) {
        ...
        // We don't call reorderMenu() here to avoid disrupting the navigation
        // The menu will be reordered on the next page load
    });
...
//     var $menuItems = $menu.children('li').not(function () {
//         return $(this).find('.menu-sub').length > 0;
//     });
...
//     function getMenuItemKey($item) {
//         // Try to get text from the div inside the a tag
//         var text = $item.find('> a div').first().text().trim();
//         if (text) return text;

//         // If no text, try to get a unique class (assuming each menu item has a unique class)
//         var classes = $item.attr('class').split(/\s+/);
//         for (var i = 0; i < classes.length; i++) {
//             if (classes[i] !== 'menu-item' && classes[i] !== 'active' && classes[i] !== 'open') {
//                 return classes[i];
//             }
//         }
...
//     function reorderMenu() {
//         $menu.css('visibility', 'hidden');
//         var $itemsToSort = $menu.children('li').detach();
...
//         $menu.append($itemsToSort).css('visibility', 'visible');
//     }
...
//         var key = getMenuItemKey($parentLi);
//         usageCounts[key] = (usageCounts[key] || 0) + 1;
//         localStorage.setItem('menuUsageCounts', JSON.stringify(usageCounts));
//         // We don't call reorderMenu() here to avoid disrupting the navigation
//         // The menu will be reordered on the next page load
    ...
    if (calendar) {
        var WAcalendar = new FullCalendar.Calendar(calendar, {
            ...
            eventClick: function (info) {
                ...
                window.location.href = info.event.extendedProps.tasks_info_url;
            }
        });
        WAcalendar.render();
    }
